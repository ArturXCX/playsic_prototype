"""
guitar_crnn — modelo CRNN para predição de lead guitar (5 frets).

Arquitetura idêntica ao BassRhythmCRNN, adaptada para a guitar solo/lead:
    Green=96, Red=97, Yellow=98, Blue=99, Orange=100  (notas Expert Clone Hero)

Nomenclatura do pipeline:
    INSTRUMENT = "guitar"      →  aba no notes.xlsx / chave em SHEET_TO_TRACK
    TRACK_NAME = "PART GUITAR" →  track do MIDI Clone Hero
    AUDIO_STEM = "guitar.ogg"  →  stem gerado pelo Demucs
...
INSTRUMENT = "guitar"        # nome da aba no xlsx e chave em excel_to_midi.SHEET_TO_TRACK
TRACK_NAME = "PART GUITAR"   # nome da track MIDI Clone Hero
AUDIO_STEM = "guitar.ogg"    # stem gerado por separa_audio.py

# ─── Mapeamento MIDI Expert → fret index ────────────────────────────────────
# Notas Expert de guitar no formato Clone Hero (Rock Band):
...
class GuitarCRNN(nn.Module):
    """CRNN para predição multi-label de eventos de lead guitar por step do grid.

    Arquitetura idêntica ao BassRhythmCRNN / DrumCRNN:
